...
class TransformerDeviceAllocator:
    ...
    def get_model_modules_info(
        self, cfg: "transformer_lens.HookedTransformerConfig"
    ) -> List[ModuleInfo]:
        """
    ...
    def _allocate_sequential(
        ...
        for block in transformer_blocks:
            ...
            if current_gpu == "cpu" or self._can_fit(
                current_gpu, block, gpu_usage_mb, gpu_memory_mb
            ):
                ...
                if current_gpu_idx >= len(available_gpus):
                    self.logger.warning(
                        "Model exceeds available GPU memory, some modules will use CPU"
                    )
                    current_gpu = "cpu"
                else:
    ...
    def _find_best_fit_gpu(
        self, module: ModuleInfo, gpu_usage_mb: Dict[str, float], gpu_memory_mb: Dict[str, float]
    ) -> str:
...
def allocate_model_devices(
    ...
    Example:
        ...
        >>> cfg = HookedTransformerConfig(n_layers=12, d_model=768, n_ctx=1024, d_head=64, n_heads=12, d_vocab=50257, d_mlp=3072, device="cuda", n_devices=2, attn_only=True)
